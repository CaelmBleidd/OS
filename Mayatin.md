...
**Аутентификация** -- сопоставление идентификатора с имеющимся в системе. Результатом является открытие сессии. Мы считаем, что если аутентификация пройдена успешно, интерфейс открывается в рамках этой сессии для пользователя.
	...
	* Многофакторная -- комбинация всех трёх вариантов идентификаторов
...
### Шифрование
Рут не знает паролей своих пользователей. [Стори про военного, который работает на литейном 4]. Да, вы можете сделать всё, что угодно,  но не сможете сделать от имени пользователей. Останутся следы, что это было сделано рутом. 
...
* /dev/ttyN -- позволяет получить доступ к какому-то терминалу, как будто буквально работаем с ним
...
**Символическая ссылка** -- создаётся файл-дырка, который хранит просто путь к настоящему файлу. Если переименовать файл в исходном каталоге, старый путь будет указывать в никуда и ссылка придёт в негодность (это нормально).  
...
## Четырнадцатая пара
### FHS
**File Hierarchy Standard** был создан, чтобы определить чёткое местоположение системных файлов. В Windows каждое приложение устанавливается в строго свой каталог, где нет никаких стандартов. В Linux любое приложение интегрируется в операционную систему, становясь её частью и размазываясь по структуре. 
...
`/boot` -- точка монтирования для самостоятельной файловой системы -- механизмы запуска ядра. 
 ...
 `/etc` -- каталог, который содержит конфигурационные файлы (аналог реестра в шинде). Для конфигурационных файлов нет никаких стандартов, open-source как есть 
...
* `/spool` -- хранит в себе все возможные очереди
