...
/* стори про сервак факультета и FreeBSD */
...
**Проблема "Читателей и писателей"** (он не гуглится, я скозал). Решение: использование общей очереди, т.е. все поступающие операции я записываю в общую очередь. Если я вижу операцию чтения, разрешаю параллельно выполняться. Дальше, если встречаю чтение, жду, пока выполнятся все прошлые чтения. Затем выполняю только одну операцию записи в один момент времени. Однако, если они все чередуются, это неэффективно. 
...
![nfts](https://i.imgur.com/ChjgDFN.png)
...
#### Виды виртуализаций
1. Полная эмуляция аппаратуры
	* ![full-emulation](https://i.imgur.com/jOIrbuM.png)
	* Виртуальная ОС с точки зрения основной -- это просто приложение, ровно такое же, как и браузер и другие. Внутри виртуальной ОС тоже есть планировщик, который только думает, что он знает о ресурсах компа, но на самом деле это не так. Эффективно планировать что-либо бессмысленно. 
	* Ещё одна проблема здесь -- вопрос, связанный с сетевой маршруторизацией. Наружу я смотрю одним IP адресом, хотя внутри существует множество адресов (вида 192.168.0.*). Это значит, что для каждого пакета нужно будет принимать решение о его перенаправлении, это долго.
	
2. Нативная виртуализация
	* ![nе **Native- virt](https://i.imgur.com/ups0lGI.png)
 uatioУ меня есть порт, который называется *hypervisor*. Дальше у меня существует множество ОС, одна хостовая, другие -- виртуальные, в них уже будут крутиться приложения. Гипервизор умеет управлять очередями и физическими прерываниями, причём алгоритмы, которые реализуются внутри гипервизора, делаются обычно очень простыми. Хостовая ОС с точки зрения разделения ресурсов обладает такими же правами, как и виртуальные. Правда гипервизор не имеет своего собственного интерфейса, поэтому только хост-система имеет право менять его файлы. 
	* Преимущество очевидно -- гипервизор для меня предсказуем. Тогда любая моя ОС получает определённый процент ресурсов, причём этот процент стабилен. 
	* Какие минусы? Это значит, что у меня гипервизор должен знать всё про ОС, с которыми он работает. Когда я использую VirtualBox и мне нужно научиться поддерживать левую ОС, нужно написать библиотеку и обновить версию VirtualBox, т.е. просто обновить приложение. Заставить же гипервизор поддерживать новую ОС сложно, к тому же, чем больше систем поддерживает гипервизор, тем больше и медленнее он становится. 

3. ПараВиртуализация
* Модель, которая появилась для решения предыдущей проблемы. Я могу менять ядра ОС так, чтобы они научились работать с гипервизором, а не наоборот. 

4. Виртуализация уровня ядра ОС
	* ![core-virt](https://i.imgur.com/2biAQqR.png)
	* У нас может быть множество задач, для которых мы создаём виртуальную ОС, но при этом я такой ортодоксальный админ, который считает, что может быть только один дистрибутив, который был создан 1488 лет назад. Что тогда получается? Затраты на ядра могут и не окупаться. Здесь и появилась идея: давайте сделаем модель, где ядро будет общее, а вот пространства функционирования приложений будут изолированы друг от друга. Т.е. каждая группа приложений считает, что они единственные и в своей ОС
	* По факту получается следующее: у нас теперь есть процесс init, порождающий ОС, но из него могут исходить некоторые другие init-процессы
	
	![init](https://i.imgur.com/iej4nMf.png)
	
	* Понятно, что чудес на бывает и мы теперь не застрахованы от сбоя, который приведёт к лавинообразному обращению к ядру, т.е. мы можем просто уронить ядро. Но при этом мы приобретаем независимость и приемлимые накладные расходы. 

#### Классификация по типу виртуализируемой среды:
1. Виртуализация представлений -- я, фактически, предоставляю только удалённый изолированный рабочий стол
2. Виртуализация рабочих мест -- у меня на той стороне полноценная ОС, только я получаю доступ к ней и её приложениям
...
3. Прозрачность расположения - пользователь не знает, где расположены ресурсы, выделенные его процессу
4. Прозрачность миграции - ресурсы могут перемещаться без изменения их имён
5. Прозрачность размножения - пользователь не знает, сколько существует копий его процесса <кулстори про космодром, старт ракеты, дублированных офицеров и полковника>
6. Прозрачность конкуренции - ничего не знаю о том, что есть другие пользователи и процессы, конкурирующие за ресурсы
7.  Прозрачность параллелизма - не знаю о том, выполняются фрагменты процесса параллельно или последовательно
...
1. Центральный сервер памяти
	...
	* Машины спрашивают у узла, где находится адрес
...
3. Размножение на чтение
	...
	* Проблема -- целостность данных -- при изменении страницы надо искать все копии и менять их тоже
	* Решения:
...
Вообще не очень понятно с развитием облачных вычислений, нужны ли распределённые, потому что 1000 машин > 1000 математиков, которые придумают модель. А может, и не придумают.



## ДвеПропущенная лекция


## надцатая пара
 ### Угрозы
 Американцами был создан orange-book, в котором были перечислены уровни безопасности. Последний, пятый, уровень был реализованля первой АЭС. На защиту системы тратилось 92% всех ресурсов системы. Стало понятно, что это не тот путь, который стоит избрать. 
...
2. Механизмы, связанные с аудитом -- логирование действий
3. Шифрование
...
1. То, что пользователь может знать (логин, пароль, кодовое слово)
2. То, чем пользователь может обладать (мобильный телефон, смарт-карта) ((короче, двухфакторка))
<к
Кулстори про яндекс и отдел, выходящий гуськом>
3. Неотъемлемая характеристика пользователя (биометрия)

**Аутентификация** -- сопоставление идентификатора с имеющимся в системе. Результатом является открытие сессии. Мы считаем, что если аутентификация пройдена успешно, интерфейс открывается в рамках этой сессии для пользователя.
	* Дискретная -- конечное количество раз (надо логиниться один раз/каждые N минут etc)
	* Непрерывная -- датчик считывания радужки например -- работать, пока пользователь сидит перед камерой
	* Многофакторная --Бывает дискретной и непрерывной. При этом сегодня всё чаще используется комбинация всех трёх вариантов идентификаторов. 

**Авторизация** -- определение разрешений на доступ к тем или иным объектам для тех или иных субъектов. 
Как правило, подразумевается работа с матрицей разрешения. Появляются вопросы: где хранить и как быстро обращаться? 
Есть два варианта: 

**Дискреционный доступ**: access control lists
1. Мы либо для объектов храним столбцы, убирая разреженность
2. Либо для субъектов храним строки
Он не позволяет защитить информацию от несанкционированного разглашения. Ничто не мешает тому, кто имеет доступ к информации, передать её тому, кто не имеет.

**Мандатный доступ**
Появился ещё во время второй мировой войны. Как устроен? 
Устроен с помощью токенов безопасности. Каждый субъект и каждый объект обладает определённым токеном. Все токены распределены по определённой оси. Любой объект получает токен, и начинает действовать следующее правило: 
я имею право получить доступ к информации, если токен данных либо совпадает с моим, либо ниже моего. Записать информацию я могу только в токены с моим или **выше** (ниже писать нельзя, чтобы не разгласить). 
Раньше его использовали, когда были очень большие опасения насчёт безопасности. Вернулись к нему только когда пришли к распределённым системам, ведь для них единая таблица доступа -- узкое место. 

В Windows вся система разрешений реализована по первой схеме. Казалось бы, что это медленно, но нет. Они жертвуют безопасностью ради быстродействия. Доступ проверяется только в первый раз, дальше на время жизни вашего процесса вам дадут мандат на доступ к нему. 

В Linux пошли по другому пути. Давайте вообще не будем считать, что у нас много субъектов. Пусть есть три субъекта: owner, group, other. 
Как правило, **owner** -- создатель, но можно владение передать с потерей всех прав владельца. 
**Group** -- группа доверенных лиц, по-умолчанию, только вы (но можно и удалить себя из него). Можно включить в него кого хочешь. 
**Other** -- все остальные. 
Хранится всё в виде трёх групп бит -- RWX для всех трёх семейств. 
При обращении к файлу проверяется, не я ли являюсь владельцем. Если да, то проверяются три первых бита (т.е. если я отобрал у себя право на чтение, то у меня действительно не получится прочитать его. При этом, если я нахожусь в группе, причём там я могу читать, всё равно ничего не получится). 

Что происходит, если это каталог? 
В Windows если я не имею доступ к каталогу, я не имею доступа ко всем файлам внутри. В Linux это не так. Я могу менять файлы, но не содержимое каталога. 
...
### Шифрование
Рут не знает паролей своих пользователей. [Стори про военногочелика, который работает на литейном 4]. Да, вы можете сделать всё, что угодно,  но не сможете сделать от имени пользователей. Останутся следы, что это было сделано рутом. 
...
ра 
 Пользователи Windows привыкли ~к говну~, что все файлы -- это просто контейнеры для некоторых данных. Конечно, мы понимаем, что системные данные тоже хранятся в файлах, но мы к этому близко не притрагиваемся. 
В Linux же само взаимодействие с системой осуществляется через файлы. 
Файла подразделяются на:
* регулярные -- любой контейнер с данными
* каталог / директория. В Windows вы можете создать внутри каталога файл с таким же названием. В Linux это не так, ведь для него каталог это такой же файл, просто этот каталог хранит в себе таблицу, в которой находится информация о содержащихся файлах. 
* дырки (file-holes). Такой файл фактически является буфером куда-то. Т.е. это не файл в типичном его понимании.

Дырки разделяются на :
* файлы физических устройств: они тоже разделяются на блочные и байт-ориентированные. 
* символьные ссылки
* именованные каналы
* сокеты 

Какие типовые файлы физических устройств могут встретиться? 
* /dev/null -- считать ничего нельзя, всё, что записывается в него, исчезает навсегда. Например, чтобы выбросить туда stderr нужно писать com 2> /dev/null
* /dev/console -- активная консоль, является указателем на один из tty
* /dev/ttyN -- позволяет получить доступ к какомуй-то терминалуконсоли, как будто буквально работаем с ним
	* `echo str > /dev/tty3` напишет str в третий терминал
* /dev/mouseN -- работа с мышкой
* /dev/ethN -- Ethernet
* /dev/sda -- контроллер диска
	* /dev/sda1 -- раздел 1
	* /dev/sdb5 -- раздел ей
* /dev/mouseN
* /dev/ethN
...
Копирование полного жёсткого диска:
```
Донор
`dd if=/dev/sda | nc -lp 12345`
Получатель 
`nc 192.168.0.1:12345 | dd of=/dev/sda`
```
Проблемы: возможна потеря данных, которые будут пытаться передаться ещё раз. К тому же, если у нас есть жёсткий диск на 2ТБ и на нём 10Мб полезных данных, почти 2ТБ мусора тоже пойдут по сети. 

В Linux запрещены жёсткие ссылки на каталоги. Зачем это сделано, если каталог тоже файл? Мы привыкли, что в Linux нет защиты от дурака. Что может быть хуже, чем удалить систему через `-rm -rf`? Я могу зациклить ссылки каталогов. Часть файловой системы либо станет вообще недоступной, либо мы потеряем возможность искать корень каталога.

**Символическая ссылка** -- создаётся файл-дырка, который хранит просто путь к настоящему файлу. ЕПроблема -- если я переименоватьл файл в исходном каталоге, старый путь будет указываеть в никуда и ссылка придёт в негодность (это нормально).  

**Именованные каналы** создают файл, который нужен для взаимодействия некоторых файлов.
Рассмотрим пайпы (`man bash | grep ...` например). За их работу отвечает родительский процесс, который буферизует данные
Можно командой `makenode`/`makefifo` создать точку доступа к этому буферу. Сценарий использования -- handler/generator из 4 лабы, только вместо диска мы пишем в буфер и это соответственно работает быстрее. 

**Сокет** -- фактически такой же именованный канал, но расположенный где-то за пределами системы, доступ к которому осуществляется через TCP-протокол. 


Когда я делаю команду `ls -l`, я получаю строчки вида:
`-rwxr-xr-- 1 user user 1024 data /.../.../file`
Что здесь написано? 
...
## ЧетырДвенадцатая пара
### FHS
**File Hierarchy Standard** был создан, чтобы определить чёткое местоположение системных файлов. В Windows каждое приложение устанавливается в строго свой каталог, где нет никаких стандартов. В Linux любое приложение интегрируется в операционную систему, становясь её частью и. Оно размазываясьжется по структуре. 

`/bin` -- изначально бинарники, сейчас уже не так. Там находятся исполняемые файлы, которые необходимо либо для работы в первом режиме, либо критично важны для запуска операционной системы.
`/sbin` -- остальные файлы, которые нужны для корректной работы ОС независимо от установленного ПО, но не относятся к критически важным. 
`/boot` -- точка монтирования для самостоятельной файловой системы -- механизмы запуска ядра. 
 `/dev` -- devices, содержит file-holes -- образуют интерфейс для всех устройств.
 `/etc` -- каталог, который содержит конфигурационные файлы (аналог реестра в шинде). Д. Нет никаких стандартов для конфигурационных файлов нет никаких стандартов, open-source как есть 
 	* `/etc/X11` -- конфигурационные файлы для графического интерфейса
 `/home` -- директория для файлов пользователей
 `/lib` -- каталог, который содержит разделяемые библиотеки, необходимые для компилятора Си
 `/lost+found` -- какая бы надёжная ни была файловая система, всё равно могут произойти сбои. В частности, может возникнуть ситуация, когда файл будет содержать атрибут, что он существует, хотя в реальности он будет удалён отовсюду. Checkdisk столкнётся с тем, что есть файлы, которые не принадлежат никакому каталогу, но в остальном кажется, что они существуют. Все эти данные система восстановит в данный каталог. При этом нужно понимать, что имя файла не будет сохранено. 
 `/media` -- для разных лазерных дисков и тд, медиа-устройств. 
 `/mut` -- монтируются жёсткие диски, которые нужны просто для хранения архивов и тд, временные пользовательские жёсткие диски. 
 `/proc` -- хранит в себе информацию, отображающую информацию о состоянии процессов и ядра ОС. Это не стандартный каталог, в него монтируется псевдофайловая система, она нигде не хранится в виде носителя. 
 `/root` -- домашний каталог superuser-а -- находится не в `/home`, потому что нормальный админ монтирует `/home` на другом жёстком диске. 
 ...
 * `/etc`
 * `/lib`
 * `/dict` -- пользовательские словари
 * `/include` -- исходники либ
 * `/include`
 * `/man` -- содержит все мануалы
 * `/src` -- исходники программ
 	* `/src/linux` --внутри есть подкаталог `/linux`, где лежит исходный код ядра. 
 * `/X11R6` -- графическая оболочка (в т.ч. исполняемые файлы, либы etc)
 * `/local`, внутри которого ещё раз можно найти все папки, которые были выше. Это нужно, чтобы размещать в каталоге только те программы, которые уникальны для данного экземпляра машины. 

`/var` -- содержит в себе: 
* `/backups` -- резервные копии системных файлов
* `/lock` -- в папке создаются ссылки на все файлы, которые сейчас открыты тем или иным процессом, чтобы не возникало коллизий. 
* `/spools` -- хранит в себе все возможные очереди
* `/log` -- каталог журналов


### Иерархия дерева процессов
Родительский процесс при убийстве получает код возврата мейна дочернего процесса, дальше можно как-то обрабатывать его.

Существует механизм удочерения -- если родительский процесс будет убит раньше дочернего, то чаще всего всех удочеряет init, но бывают и другие варианты, например, усыновление "дедом". 

Любой процесс порождается либо парой системных вызовов `fork() exec()`, либо ...
Если вы вызываете `fork()`, создаётся полная копия процесса. Это существует, чтобы обеспечить преемственность прав. Дальше код приложения подменится, но подменится он уже в `exec()`. В последний момент `fork()` поменяет `PID` процесса, при этом будут два процесса с разными пидами, но идентичным содержимым, новый процесс будет игнорироваться планировщиком, пока не будет выполнено `exec()`. 
Что нужно понимать? В этой модели невозможно реализовать многопоточность. 

Системный вызов `clone()` атомарно выполняется `fork+exec`, при этом в флаге мы можем указать что-то о разделении адресного пространства родителя. 

Любой процесс, который запустился в результате, может находиться в одном из состояний: 
* R -- running
* S -- sleeping
* T -- трассировка
* D -- непрерываемое ожидание, т.е. процесс ждёт чего-то конкретного и только этого
* Z -- zombie

Ядро это не процесс, это совокупность потоков ядра, которые нельзя называть процессами, потому что они имеют ряд отличий. 
Сначала, когда мы замкнули питание, отработал код интеллектуального загрузчика на материнской плате. Отработал post, который проверяет состояние узла. Если пост благополучно пройдет, т.е. все устройства и шины ответили корректно, то нам необходимо загружать ОС. Для этого нужно её найти. Происходит опрос жёстких дисков (для SATA в порядке портов), ищется загрузчик. В Linux сейчас стандартный загрузчик **grub**. Он состоит из трёх компонентов: `stage1`, `stage1.5`, `stage2` Как это устроено? 
`Stage1` записан в `MBR`, который содержит код загрузчика операционной системы. 
Сейчас всё, что `stage1` умеет -- находить `stage1.5` -- карту на диске, где искать драйвер для монтирования файловой системы. 
В чём идея? У меня есть ядро в виде бинарного файла в файловой системе. Нам нужен драйвер файловой системы, но это компонент ядра. Нам нужно запустить ядро, для этого нужно иметь драйвер файловой системы, т.е. компонент ядра, для чего нужно ядро. По сути, `stage1.5` это драйвер файловой системы, который должен смонтировать файл файловую систему, который, с точки зрения ОС, находится в `/boot` (??????). Появляется `/boot/grub/stage2`. (?????) Что-то связанное с `/menu.lst`, `grub.conf` (???????). 
Как устроена запись внутрь grub.conf? Внутри будет стоять слово `kernel` и путь, например:
...
## Пропущена лекция (153)
Она и была последней ;( 
<!--stackedit_data:
eyJoaXN0b3J5IjpbOTY0MjYxMzY2LDE3MjQ5NTAxMzgsNTMwNj
M3Nzg5LDE2NTY3MDE4MjUsMTgyOTk1Mzk1LC0yMDcxNjAyNTks
MTIzNTA0ODk0NiwtMzgzNjUyNjI2LDkwNDUzODYwOCwxNTk2Mz
