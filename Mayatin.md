...
Решение получило название **Native virtualization**
...
Пара-Виртуализация
Модель, которая появилась для решения этой проблемы. Я могу менять ядра ОС так, чтобы они научились работать с гипервизором, а не наоборот. 

Следующий путь -- виртуализация уровня ядра ОС
(**И СЮДА ФОТОЧКУ**)
У нас может быть множество задач, для которых мы создаём виртуальную ОС, но при этом я такой ортодоксальный админ, который считает, что может быть только один дистрибутив, который был создан 1488 лет назад. Что тогда получается? Затраты на ядра могут и не окупаться. Здесь и появилась идея: давайте сделаем модель, где ядро будет общее, а вот пространства функционирования приложений будут изолированы друг от друга. Т.е. каждая группа приложений считает, что они единственные и в своей ОС. Как это устроено? 
По факту получается следующее: у нас теперь есть процесс init, порождающий ОС, но из него могут исходить некоторые другие init-процессы (**ЗАЛИТЬ СЮДА ФОТОЧКУ**). Понятно, что чудес на бывает и мы теперь не застрахованы от сбоя, который приведёт к лавинообразному обращению к ядру, т.е. мы можем просто уронить ядро. Но при этом мы приобретаем независимость и приемлимые накладные расходы. 
...
3. Неотъемлемая характеристика пользователя
...
Бывает дискретной и непрерывной. При этом сегодня всё чаще используется комбинация всех трёх вариантов идентификаторов. 
...
* /dev/ttyN -- позволяет получить доступ к какой-то консоли, как будто буквально работаем с ней
* /dev/mouseN
* /dev/ethN
* /dev/sda
* /dev/sda1
...
**Символическая ссылка** -- создаётся файл-дырка, который хранит просто путь к настоящему файлу. Проблема -- если я переименовал файл в исходном каталоге, старый путь указывает в никуда.  
...
## Двенадцатая пара
### FHS
**File Hierarchy Standard** был создан, чтобы определить чёткое местоположение системных файлов. В Windows каждое приложение устанавливается в строго свой каталог, где нет никаких стандартов. В Linux любое приложение интегрируется в операционную систему, становясь её частью. Оно размажется по структуре. 
...
`/sin` -- остальные файлы, которые нужны для корректной работы ОС независимо от установленного ПО, но не относятся к критически важным. 
`/boot` -- точка монтирования для самостоятельной файловой системы. 
 `/dev` -- devices, содержит file-holes.
 `/etc` -- каталог, который содержит конфигурационные файлы. Нет никаких стандартов для конфигурационных файлов, open-source как есть 
 ...
 `/root` -- находится не в `/home`, потому что нормальный админ монтирует `/home` на другом жёстком диске. 
 ...
 * `/src` -- внутри есть подкаталог `/linux`, где лежит исходный код ядра. 
 * `/X11R6`
 * `/local`, внутри которого ещё раз можно найти все папки, которые были выше. Это нужно, чтобы размещать в каталоге только те программы, которые уникальны для данного экземпляра. 
